...
#Training. every point takes about 10 seconds to complete
...
ggplot() +
  geom_line(aes( x = date_daily, y = ts(replacee_na(as.double(elspot_prices_2021_daily_eur$...24[-1]),stand=FALSE), start = 1 ) , colour = "Observed Values")) +
  theme(legend.title = element_blank() ) + xlab("") + ylab("Daily Prices of Crude Oil in USD/barrel") +
  geom_ribbon(aes(x = date_daily[1:91], ymin = ts(rep(50,91),start=FS) , ymax = ts(rep(100,91),start=FS)) , alpha = 0.2 , fill = "blue") +
  geom_ribbon(aes(x = date_daily[91:183], ymin = ts(rep(50,93),start=FS) , ymax = ts(rep(100,93),start=FS)) , alpha = 0.2 , fill = "green") +
  geom_ribbon(aes(x = date_daily[183:276], ymin = ts(rep(50,94),start=FS) , ymax = ts(rep(100,94),start=FS)) , alpha = 0.2 , fill = "yellow") +
  geom_ribbon(aes(x = date_daily[276:365], ymin = ts(rep(50,90),start=FS) , ymax = ts(rep(100,90),start=FS)) , alpha = 0.2 , fill = "red") +
  theme(legend.title = element_blank() ) + xlab("") + ylab("Daily Prices in Eur/Mwh") +
  scale_color_manual(values = c("black", "red" ))
